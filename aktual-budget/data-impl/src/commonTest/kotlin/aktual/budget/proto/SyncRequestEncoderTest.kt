...
class SyncRequestEncoderTest {
  ...
  fun `Encode unencrypted empty sync request`() = runTest {
    ...
    assertThat(encoded.base64()).isEqualTo(
      RESOURCES_DIR
        .resolve("proto-request-unencrypted-empty.txt")
        .readLines()
        .first(),
    )
  }
  ...
  fun `Encode unencrypted add one row request`() = runTest {
    ...
    val messages = listOf(
      ...
      Message(dataset, row, "description", time2(0), MsgStr("c1205fa3-b69f-460e-94e0-5550e2098cc3")),
    ...
    assertThat(encoded.base64()).isEqualTo(
      RESOURCES_DIR
        .resolve("proto-request-unencrypted-added.txt")
        .readLines()
        .first(),
    )
  }
  ...
  fun `Encode encrypted new local account request`() = runTest {
    ...
    assertThat(encoded.base64()).isEqualTo(
      RESOURCES_DIR
        .resolve("proto-request-encrypted-new-account.txt")
        .readLines()
        .first(),
    )
  }
  ...
  fun `Encode encrypted delete local account request`() = runTest {
    ...
    assertThat(encoded.base64()).isEqualTo(
      RESOURCES_DIR
        .resolve("proto-request-encrypted-delete-account.txt")
        .readLines()
        .first(),
    )
  }
  ...
  private fun buildEncoder(
    ...
    val itr = iv.iterator()
    val random = mockk<Random> { every { nextBytes(size = any()) } answers { itr.next().base64().toByteArray() } }
