...
abstract class GenerateReadmeTask : DefaultTask() {
  ...
  fun execute() {
    ...
    val readmeContents = buildString {
      appendLine("# $thisPath")
      appendLine("> :warning: This readme is autogenerated with `gradle $TASK_NAME`. Do not edit manually! :warning:")
      appendLine()
      append(generateLegendContents(usedTypes))
      appendLine()
      appendLine("```mermaid")
      append(generateMermaidContents(links, nodes))
      appendLine("```")
    }
...
private fun generateLegendContents(usedTypes: Set<ModuleType>): String = buildString {
  appendLine("| Module Type | Colour |")
  appendLine("|:--:|:--:|")

  for (type in usedTypes) {
    val url = "https://img.shields.io/badge/-%20-${type.color.removePrefix("#")}?style=flat-square"
    val img = "<img src=\"$url\" height=\"20\" width=\"80\">"
    appendLine("| ${type.label} | $img |")
  }
}
