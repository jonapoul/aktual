...
class KeyPreferencesImpl(private val prefs: EncryptedPreferences) : KeyPreferences {
  override operator fun contains(keyId: KeyId?): Boolean = keyId?.let { prefs.contains(key(it)) } == true
  ...
  override suspend fun setAndCommit(keyId: KeyId, value: ByteString) = preference(keyId).setAndCommit(value)
