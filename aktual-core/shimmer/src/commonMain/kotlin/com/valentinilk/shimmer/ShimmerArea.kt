...
internal class ShimmerArea(private val widthOfShimmer: Float, rotationInDegree: Float) {
  ...
  /**
   * Rotating the shimmer results in an effect that will first be visible in one of the corners. It
   * will afterwards travel across the view / display until the last visible part of it will
   * disappear in the opposite corner.
   *
   * A simple shimmer going across the device's screen from left to right has to travel until it
   * reaches the center of the screen and then the same distance again. Without taking the shimmer's
   * own width into account.
   *
   ...
   * from the top left of the display to the rotated shimmer (or center line), hitting it at a 90
   * degree angle. As the height and width of the display (or view) are known, the length of the
   * line can be calculated by using basic trigonometric functions.
   */
  ...
  /**
   * The formula to compute the required distance for the shimmer's traversal across the view only
   * accepts inputs between 0 and 90. All inputs above 90 can simply be mapped to a value in the
   * accepted range. Inputs between 90 and 180 have to be mapped to the values between 90 and 0
